...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // health bar
        healthBar = FindObjectOfType<PlayerHealth>();
    ...
    public virtual void Attack() {
        ...
        // attack in any state or idle in grounded state its up to us
        if (lockMovement || !isGrounded) return;

        animator.ResetTrigger("Attack_"+attackID);
        lockMovement = true;
        _rigidbody.velocity = Vector3.zero;
        attackID = Random.Range(1,3);
        
        animator.SetTrigger("Attack_"+attackID);
    ...
    public virtual void UpdateAnimator()
    {
        ...
        lockMovement = animator.GetCurrentAnimatorStateInfo(0).IsName("Attack_" + attackID);
        swordColldier.enabled = lockMovement;

        animator.SetFloat(AnimatorParameters.Attack, attackID);
